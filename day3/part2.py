...
def find_the_match(route1, route2):
	return[i for i in route1 if i in route2]
...
def closest_range(result,route1,route2):
	results = []
	for item in result:
		count = 0
		for cord1 in route1:
...
route1 = populate_lists(list1)
route2 = populate_lists(list2)
result = find_the_match(route1,route2)

#Solution Part 1:
#print("Manhattan number of nearest intersect: " + str(manhattan(result)))

#Solution Part 2:
results = closest_range(result,route1,route2)
results.sort()
print("Shortest trip is: " + str(results[:1]))